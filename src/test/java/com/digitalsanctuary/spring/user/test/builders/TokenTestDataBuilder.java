...
/**
 * Fluent builder for creating test token entities (VerificationToken and PasswordResetToken). This builder simplifies token creation for tests with
 * various expiration states.
 *
 * Example usage:
 *
 * <pre>
 * VerificationToken validToken = TokenTestDataBuilder.aVerificationToken().forUser(testUser).expiringInMinutes(60).build();
 *
 * PasswordResetToken expiredToken = TokenTestDataBuilder.aPasswordResetToken().forUser(testUser).expired().build();
 * </pre>
 */
public class TokenTestDataBuilder {
    ...
    public static VerificationTokenBuilder aValidVerificationToken() {
        return new VerificationTokenBuilder().expiringInHours(24);
    }
    ...
    public static VerificationTokenBuilder anExpiredVerificationToken() {
        return new VerificationTokenBuilder().expired();
    }
    ...
    public static PasswordResetTokenBuilder aValidPasswordResetToken() {
        return new PasswordResetTokenBuilder().expiringInHours(1);
    }
    ...
    public static PasswordResetTokenBuilder anExpiredPasswordResetToken() {
        return new PasswordResetTokenBuilder().expired();
    }
}
